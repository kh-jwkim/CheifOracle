...
/*
    ...
    - SELECT 쿼리의 실행 결과를 화면에 저장한 논리적인 가상의 테이블 (실질적인 데이터를
    - 저장하고 있지 않지만 하나의 테이블처럼 사용 가능함)
    
    1.2 종류
    1.2.1 Store View(저장o) - 이름을 붙여서 저장함
    1.2.2 Inline View(저장x) - FROM 절 뒤어 적는 서브쿼리
*/
-- 1.2.1 Store View(저장o) - 이름을 붙여서 저장함
...
GRANT CREATE VIEW TO KH;    --SYSTEM 계정으로 권한 부여
-- View 결과 확인
...
/*
    ...
    - 특정 사용자가 원본 테이블에 접근하여 모든 데이터를 보게 하는 것을 방지
*/

-- ex) 연봉정보를 가지고 있는 VIEW를 생성하시오.(ANNUAL_SALARY_VIEW)
-- 사번, 이름, 급여, 연봉
CREATE VIEW ANNUAL_SALARY_VIEW(사번,이름,급여,연봉)
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY*12+NVL(BONUS,0)*SALARY "SALARY_Y"
FROM EMPLOYEE;
...
UPDATE V_EMPLOYEE
SET DEPT_CODE = 'D8' WHERE EMP_NAME='선동일';
...
-- 1.5.2 FORCE/NOFORCE
-- 기본 테이블이 존재하지 않더라도 뷰를 생성하는 옵션 (쓸일이 거의 없음.. 가끔 테스트용으로나 사용)

-- 1.5.3 WITH CHECK OPTION
-- WHERE 조건에 사용한 컬럼의 값을 수정하지 못하게 하는 옵션

-- 1.5.4 WITH READ ONLY
-- VIEW에 대한 조회만 가능하며 DML 불가능하게 하는 옵션
CREATE OR REPLACE VIEW V_EMPLOYEE
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, NVL(DEPT_TITLE, '미정') "DEPT_TITLE", 
NVL((SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = E.MANAGER_ID), '없음') "MANAGER_NAME", HIRE_DATE
FROM EMPLOYEE E
...
-- 데이터 딕셔너리 안에는 중요한 정보가 많이 있기 때문에 사용자는 이를 활요하기
...
SELECT SEQ_KH_USER_NO.CURRVAL FROM DUAL; -- NEXTVAL를 실행한 수 현재 시퀀스값 조회
...
CREATE SEQUENCE SEQ_MEM_COM
START WITH 1
INCREMENT BY 1
NOCYCLE
NOCACHE;

INSERT INTO KH_MEMBER VALUES(SEQ_MEM_ID.NEXTVAL, '홍길동', 20, SEQ_MEM_COM.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SEQ_MEM_ID.NEXTVAL, '청길동', 30, SEQ_MEM_COM.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SEQ_MEM_ID.NEXTVAL, '외길동', 40, SEQ_MEM_COM.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SEQ_MEM_ID.NEXTVAL, '고길동', 50, SEQ_MEM_COM.NEXTVAL);

SELECT * FROM KH_MEMBER;


-- 2.4 시퀀스 수정
-- ALTER를 이용해서 옵션들을 수정할 수 있다
ALTER SEQUENCE SEQ_MEM_ID
-- 다만 START WITH 변경 불가!! -> 삭제 후 다시 생성
INCREMENT BY 1
MAXVALUE 99999;
