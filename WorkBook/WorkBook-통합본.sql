...
SELECT EXTRACT(YEAR FROM TO_DATE('51/10/11','YY/MM/DD')) FROM DUAL;
SELECT EXTRACT(YEAR FROM TO_DATE('51/10/11','RR/MM/DD')) FROM DUAL;
...
SELECT * FROM TB_CLASS WHERE CLASS_NAME = '디자인평가론';
...
-- 최근 3 년을 기준으로 수강인원이 가장 많았던 3 과목을 찾는 구문을 작성해보시오. 과목번호 과목이름 누적수강생수(명)

DESC TB_CLASS;
DESC TB_GRADE;

SELECT CLASS_NO, CLASS_NAME FROM TB_GRADE
LEFT OUTER JOIN TB_CLASS USING(CLASS_NO)
ORDER BY CLASS_NO ASC;

SELECT TO_CHAR(ADD_MONTHS(TO_DATE(SELECT * FROM (SELECT TERM_NO FROM TB_GRADE ORDER BY TERM_NO DESC) WHERE ROWNUM=1),-36), 'YYYYMM') FROM DUAL;

SELECT * FROM (SELECT CLASS_NO, CLASS_NAME, COUNT(*) FROM (SELECT * FROM TB_GRADE WHERE TERM_NO >= '200603')
LEFT OUTER JOIN TB_CLASS USING(CLASS_NO)
GROUP BY CLASS_NO, CLASS_NAME ORDER BY 3 DESC)
WHERE ROWNUM <= 3;
...
-- 6.주민등록번호 보호법에 따라 학생정보 테이블에서 주민번호 뒷자리를 저장하지 않기로 결정하였다. 이 내용을 반영할 적절한 SQL 문장을 작성하시오. (예. 830530-2124663 ==> 830530)
