...
-- 계정에 부여된 롤과 권한 확인
SELECT * FROM USER_ROLE_PRIVE;
SELECT * FROM USER_SYS_PRIVS;

-- 4. INDEX
-- 4.1 개요
-- - SQL명령문의 처리속도를 향상시키기 위해서 컬럼에 대해서 생성하는 오라클 객체
-- - KEY와 VALUE형태로 생성이 되며 KEY에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된 주소값이 저장
...
--  > 전체 데이터 중에 10 ~ 10% 이내의 데이터를 검색하는 경우, 중복이 많지 않은 컬럼이어야 함.
