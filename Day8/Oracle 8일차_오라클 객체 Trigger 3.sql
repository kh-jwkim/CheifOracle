...
VALUES((SELECT MAX(EMP_ID)+1 FROM EMPLOYEE), '일용자', '000624-3223444', '01012345678', 'J5', 'S5');
...
CREATE OR REPLACE TRIGGER TRG_UP_BEF
BEFORE
UPDATE ON EMPLOYEE
FOR EACH ROW
BEGIN
    DBMS_OUTPUT.PUT_LINE(:OLD.SALARY || ' BEF ' || :NEW.SALARY);
END;
/

CREATE OR REPLACE TRIGGER TRG_UP_AFT
AFTER
UPDATE ON EMPLOYEE
FOR EACH ROW
BEGIN
    DBMS_OUTPUT.PUT_LINE(:OLD.SALARY || ' AFT ' || :NEW.SALARY);
END;
/

SELECT * FROM EMPLOYEE WHERE EMP_ID = '200';
SELECT * FROM EMPLOYEE WHERE EMP_NAME = '방명수';
UPDATE EMPLOYEE SET SALARY = 6000000 WHERE EMP_ID = '200';

UPDATE EMPLOYEE SET DEPT_CODE = 'D9' WHERE EMP_NAME = '방명수';
ROLLBACK;
...
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- UPDATE로 SALARY가 변경된 경우에만 실행!
        THEN
...
UPDATE EMPLOYEE SET JOB_CODE = 'J1' WHERE EMP_ID = '211';
-- TRIGGET로 정보가 나오지 않으면 COMMIT 해보기(메모)
-- COMMIT;
ROLLBACK;
...
CREATE TABLE PRODUCT_IO(
    ...
    PCODE NUMBER, -- CONSTRAINT FK_PRODUCT_IO REFERENCES PRODUCT(PCODE), -> COLUMN 레벨 에서 FOREIGN KEY 설정
    PDATE DATE,
    AMOUNT NUMBER,
    STATUS VARCHAR2(10) CHECK(STATUS IN('입고','출고')),
    CONSTRAINT FK_PRODUCT_IO FOREIGN KEY(PCODE) REFERENCES PRODUCT(PCODE) --> TABLE 레벨 에서 FOREIGN KEY 설정
);
...
FOR EACH ROW
BEGIN
    IF(:NEW.STATUS = '입고') -- STATUS가 '입고' 인 경우에만 실행!
    THEN
        UPDATE PRODUCT SET STOCK = STOCK + :NEW.AMOUNT WHERE PRODUCT.PCODE = :NEW.PCODE;
    ELSIF(:NEW.STATUS = '출고')
    THEN
        UPDATE PRODUCT SET STOCK = STOCK - :NEW.AMOUNT WHERE PRODUCT.PCODE = :NEW.PCODE;
    END IF;
