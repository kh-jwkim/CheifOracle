...
-- EXCEPTION
-- END;
-- / -> 이건 블록문법 여러개인 경우에 구문이 구분되도록 하기위해 꼭 필요함
SET SERVEROUTPUT ON;

DECLARE 
    --SALLARY EMPLOYEE.SALARY%TYPE;
...
-- IF (조건식) THEN (실행문1) ELSIF (조건식2) THEN (실행문2) ELSE (실행문3) END IF;
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = '&사번';
    IF (EINFO.SALARY < 990000)
        THEN SLEVEL := 'F';
    ELSIF (EINFO.SALARY BETWEEN 1000000 AND 1990000)
        THEN SLEVEL := 'E';
    ELSIF (EINFO.SALARY BETWEEN 2000000 AND 2990000)
        THEN SLEVEL := 'D';
    ELSIF (EINFO.SALARY BETWEEN 3000000 AND 3990000)
        THEN SLEVEL := 'C';
    ELSIF (EINFO.SALARY BETWEEN 4000000 AND 4990000)
        THEN SLEVEL := 'B';
    ELSE SLEVEL := 'A';
    END IF;
    
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EINFO.EMP_ID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || EINFO.EMP_NAME);
    DBMS_OUTPUT.PUT_LINE('급여 : ' || EINFO.SALARY);
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = '&사번';
    SALARY := FLOOR(EINFO.SALARY/1000000);
    CASE SALARY
...
BEGIN
    ...
    THEN
        ...
        END LOOP;
    ELSE
        DBMS_OUTPUT.PUT_LINE('2~9사이의 수를 입력하세요');
    END IF;
END;
/
...
BEGIN
    WHILE(N<=30)
    LOOP
        IF MOD(N,2) = 1 THEN DBMS_OUTPUT.PUT_LINE(N); END IF;
        N := N+1;
    END LOOP;
END;
/
...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('사번 : ' || EINFO.EMP_ID);
        DBMS_OUTPUT.PUT_LINE('이름 : ' || EINFO.EMP_NAME);
        DBMS_OUTPUT.PUT_LINE('메일 : ' || EINFO.EMAIL);
    END LOOP;
END;
/
...
BEGIN
    ...
    LOOP
        ...
        VALUES(TRUNC(DBMS_RANDOM.VALUE(0,99)), SYSDATE-TRUNC(DBMS_RANDOM.VALUE(0,7)));
    END LOOP;
END;
/
